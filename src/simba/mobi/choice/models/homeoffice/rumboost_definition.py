...
def define_variables(df_zp, choice_var_name) -> pd.DataFrame:
    ...
    new_df['has_driving_licence_not_NA'] = (df_zp['has_driving_licence'] == 1).astype(int)
    new_df['work_time_flexibility_NA'] = (df_zp['work_time_flexibility'] < 0).astype(int)
    new_df['work_time_flexibility_not_NA_fixed'] = (df_zp['work_time_flexibility'] == 1).astype(int)
    # new_df['work_parking_NA'] = (df_zp['parking_place_at_work'] < 0).astype(int)
    new_df['work_parking_not_NA_free'] = (df_zp['parking_place_at_work'] == 1).astype(int)
    new_df['is_swiss'] = (df_zp['nation'] == 8100).astype(int)
    # new_df['typology_work_NA'] = (df_zp['urban_typology_work'] < 0).astype(int)
    new_df['typology_work_not_NA_urban'] = (df_zp['urban_typology_work'] == 1).astype(int)
    new_df['typology_work_not_NA_rural'] = (df_zp['urban_typology_work'] == 2).astype(int)
